...
| **Error Example** | `400 {"detail": "Пароли не совпадают", "code": "PASSWORD_MISMATCH"}` |
...
| Method | URL | Description | Request / Response |
|---|---|---|---|
| `GET` | `/items` | Каталог вещей с фильтрацией и поиском | **Req:** Cookie: `session_id=abc123` (для `is_liked`/`is_favorited`)<br>`?category_id=uuid&style_id=uuid&brand_id=uuid&search=leather&page=1&limit=20`<br>**Res:** `200 {"items": [{"id": "uuid", "name": "Leather Sneakers", "category": {"id": "uuid", "name": "shoes"}, "style": {"id": "uuid", "name": "minimalist"}, "brand": {"id": "uuid", "name": "Nike"}, "image_url": "...", "is_liked": false, "is_favorited": false}], "total": 45, "page": 1}`<br>**Error:** `404 {"detail": "Категория не найдена", "code": "NOT_FOUND"}` |
| `GET` | `/items/{item_id}` | Детали вещи | **Req:** Cookie: `session_id=abc123` (для `is_liked`/`is_favorited`)<br>**Res:** `200 {"id": "uuid", "name": "Leather Sneakers", "description": "...", "category": {"id": "uuid", "name": "shoes"}, "style": {"id": "uuid", "name": "minimalist"}, "brand": {"id": "uuid", "name": "Nike"}, "image_urls": ["...", "..."], "is_liked": false, "is_favorited": false}`<br>**Error:** `404 {"detail": "Вещь не найдена", "code": "NOT_FOUND"}` |
| `POST` | `/items/{item_id}/like` | Лайк вещи (Free Mode swipe) | **Req:** Cookie: `session_id=abc123`<br>**Res:** `200 {"message": "Добавлен лайк", "likes_count": 12}` |
| `DELETE` | `/items/{item_id}/like` | Отмена лайка вещи | **Req:** Cookie: `session_id=abc123`<br>**Res:** `200 {"message": "Лайк удален", "likes_count": 11}` |
...
| `GET` | `/catalog/brands` | Список брендов | **Req:** `?page=1&limit=100`<br>**Res:** `200 {"brands": [{"id": "uuid", "name": "Nike"}, {"id": "uuid", "name": "Zara"}], "total": 50, "page": 1}` |
...
| `GET` | `/outfits/{outfit_id}` | Детали аутфита | **Req:** Cookie: `session_id=abc123` (для `is_liked`/`is_saved`)<br>**Res:** `200 {"id": "uuid", "author": {...}, "title": "...", "description": "...", "style": "minimalist", "published_at": "...", "items": [{"item_id": "uuid", "name": "...", "image_url": "..."}], "likes_count": 12, "is_liked": false, "is_saved": false}`<br>**Error:** `404 {"detail": "Аутфит не найден", "code": "NOT_FOUND"}` |
...
| `PUT` | `/outfits/{outfit_id}` | Редактирование черновика | **Req:** Cookie: `session_id=abc123`<br>`{"items": ["item_id_1"], "title": "Updated look", "description": "..."}`<br>**Res:** `200 {"message": "Аутфит обновлен"}`<br>**Error:** `404 {"detail": "Аутфит не найден", "code": "NOT_FOUND"}` |
...
| `POST` | `/favorites/items/{item_id}` | Добавить вещь в избранное | **Req:** Cookie: `session_id=abc123`<br>**Res:** `200 {"message": "Вещь добавлена в избранное"}` |
| `DELETE` | `/favorites/items/{item_id}` | Удалить вещь из избранного | **Req:** Cookie: `session_id=abc123`<br>**Res:** `200 {"message": "Вещь удалена из избранного"}` |
...
| `POST` | `/favorites/styles/{style_id}` | Добавить стиль в избранное | **Req:** Cookie: `session_id=abc123`<br>**Res:** `200 {"message": "Стиль добавлен"}`<br>**Error:** `409 {"detail": "Стиль уже в избранном", "code": "ALREADY_IN_FAVORITES"}` |
| `GET` | `/favorites/brands` | Избранные бренды | **Req:** Cookie: `session_id=abc123`<br>**Res:** `200 {"brands": [{"id": "uuid", "name": "Nike"}]}` |
| `POST` | `/favorites/brands/{brand_id}` | Добавить бренд в избранное | **Req:** Cookie: `session_id=abc123`<br>**Res:** `200 {"message": "Бренд добавлен"}`<br>**Error:** `409 {"detail": "Бренд уже в избранном", "code": "ALREADY_IN_FAVORITES"}` |
...
| Method | URL | Description | Request / Response |
|---|---|---|---|
| `POST` | `/profile/follow/{user_id}` | Подписаться на пользователя | **Req:** Cookie: `session_id=abc123`<br>**Res:** `200 {"message": "Вы подписаны"}` |
| `DELETE` | `/profile/follow/{user_id}` | Отписаться | **Req:** Cookie: `session_id=abc123`<br>**Res:** `200 {"message": "Вы отписались"}` |
...
| Method | URL | Description | Request / Response |
|---|---|---|---|
| `POST` | `/admin/items` | Создание вещи | **Req:** `multipart/form-data: name, category_id, style_id, brand_id, description, tags[]`<br>**Res:** `201 {"message": "Вещь создана", "item_id": "uuid"}` |
| `PUT` | `/admin/items/{item_id}` | Обновление вещи | **Req:** `{"name": "New Name", "category_id": "uuid"}`<br>**Res:** `200 {"message": "Вещь обновлена"}` |
...
| `POST` | `/admin/brands` | Создание бренда | **Req:** `{"name": "H&M"}`<br>**Res:** `201 {"message": "Бренд создан"}` |
| `GET` | `/admin/users` | Список пользователей | **Req:** Cookie: `session_id=admin_session`<br>`?page=1&limit=20`<br>**Res:** `200 {"users": [...], "total": 100, "page": 1}`<br>**Error:** `403 {"detail": "Доступ запрещен", "code": "FORBIDDEN"}` |
| `POST` | `/admin/users/{user_id}/block` | Блокировка пользователя | **Req:** Cookie: `session_id=admin_session`<br>**Res:** `200 {"message": "Пользователь заблокирован"}`<br>**Error:** `403 {"detail": "Доступ запрещен", "code": "FORBIDDEN"}` |
| `POST` | `/admin/users/{user_id}/unblock` | Разблокировка пользователя | **Req:** Cookie: `session_id=admin_session`<br>**Res:** `200 {"message": "Пользователь разблокирован"}`<br>**Error:** `403 {"detail": "Доступ запрещен", "code": "FORBIDDEN"}` |
| `DELETE` | `/admin/users/{user_id}` | Удаление пользователя | **Req:** Cookie: `session_id=admin_session`<br>**Res:** `200 {"message": "Пользователь удален"}`<br>**Error:** `403 {"detail": "Доступ запрещен", "code": "FORBIDDEN"}` |
| `DELETE` | `/admin/outfits/{outfit_id}` | Удаление публикации | **Req:** Cookie: `session_id=admin_session`<br>**Res:** `200 {"message": "Публикация удалена"}`<br>**Error:** `403 {"detail": "Доступ запрещен", "code": "FORBIDDEN"}` |
